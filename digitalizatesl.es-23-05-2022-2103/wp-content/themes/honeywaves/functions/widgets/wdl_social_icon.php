<?php // Register and load the widget
	function honeywaves_social_icon_widget() {
	    register_widget( 'honeywaves_social_icon_widget' );
	}
	add_action( 'widgets_init', 'honeywaves_social_icon_widget' );

	// Creating the widget
	class honeywaves_social_icon_widget extends WP_Widget {
	 
	function __construct() {
		parent::__construct(
			'honeywaves_social_icon_widget', // Base ID
			esc_html__('HoneyWaves: Social icons','honeywaves'), // Widget Name
			array(
				'classname' => 'honeywaves_social_icon_widget',
				'description' => esc_html__('HoneyWaves social icons widget','honeywaves'),
			),
			array(
				'width' => 300,
			)
		...
		/* enqueue script  */
			add_action( 'admin_enqueue_scripts', array( $this, 'honeywaves_social_icon_style' ) );
	 }
	public function honeywaves_social_icon_style(){?> 
		<style type="text/css">
	...
	?>
	<!-- Facebook Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'facebook_link' )); ?>"><?php esc_html_e( 'Facebook URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'facebook_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Twitter Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'twitter_link' )); ?>"><?php esc_html_e( 'Twitter URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'twitter_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	
	<!-- Lindedin Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'linkedin_link' )); ?>"><?php esc_html_e( 'LinkedIn URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'linkedin_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Instagram Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'instagram_link' )); ?>"><?php esc_html_e( 'Instagram URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'instagram_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Youtube Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'youtube_link' )); ?>"><?php esc_html_e( 'Youtube URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'youtube_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Skype Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'skype_link' )); ?>"><?php esc_html_e( 'Skype URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'skype_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Vimeo Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'vimeo_link' )); ?>"><?php esc_html_e( 'Vimeo URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'vimeo_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	
	<!-- Dribble Link -->
	<h4 for="<?php echo esc_attr($this->get_field_id( 'dribble_link' )); ?>"><?php esc_html_e( 'Dribble URL','honeywaves' ); ?></h4>
	...
	<label for="<?php echo esc_attr($this->get_field_id( 'dribble_target' )); ?>"><?php esc_html_e( 'Open link in new tab','honeywaves' ); ?></label>
	...
	$instance = array();
		...
		$instance['facebook_target'] = ( ! empty( $new_instance['facebook_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['facebook_target'] ) : '';
		
		$instance['twitter_link'] = ( ! empty( $new_instance['twitter_link'] ) ) ? esc_url_raw( $new_instance['twitter_link'] ) : '';
		$instance['twitter_target'] = ( ! empty( $new_instance['twitter_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['twitter_target'] ) : '';
		
		$instance['linkedin_link'] = ( ! empty( $new_instance['linkedin_link'] ) ) ? esc_url_raw( $new_instance['linkedin_link'] ) : '';
		$instance['linkedin_target'] = ( ! empty( $new_instance['linkedin_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['linkedin_target'] ) : '';
		
		$instance['instagram_link'] = ( ! empty( $new_instance['instagram_link'] ) ) ? esc_url_raw( $new_instance['instagram_link'] ) : '';
		$instance['instagram_target'] = ( ! empty( $new_instance['instagram_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['instagram_target'] ) : '';
		
		$instance['youtube_link'] = ( ! empty( $new_instance['youtube_link'] ) ) ? esc_url_raw( $new_instance['youtube_link'] ) : '';
		$instance['youtube_target'] = ( ! empty( $new_instance['youtube_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['youtube_target'] ) : '';
		
		$instance['skype_link'] = ( ! empty( $new_instance['skype_link'] ) ) ? esc_url_raw( $new_instance['skype_link'] ) : '';
		$instance['skype_target'] = ( ! empty( $new_instance['skype_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['skype_target'] ) : '';
		
		$instance['vimeo_link'] = ( ! empty( $new_instance['vimeo_link'] ) ) ? esc_url_raw( $new_instance['vimeo_link'] ) : '';
		$instance['vimeo_target'] = ( ! empty( $new_instance['vimeo_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['vimeo_target'] ) : '';
		
		$instance['dribble_link'] = ( ! empty( $new_instance['dribble_link'] ) ) ? esc_url_raw( $new_instance['dribble_link'] ) : '';
		$instance['dribble_target'] = ( ! empty( $new_instance['dribble_target'] ) ) ? honeywaves_sanitize_checkbox( $new_instance['dribble_target'] ) : '';
